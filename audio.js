// Mario Knight Silksong - Web Audio Synthesizer
class SoundEngine {
  constructor() {
    this.ctx = null;
    this.muted = false;
  ...
  toggleMute() {
    ...
    if (this.muted) {
      ...
      this.init();
      if (this.currentTheme && this.currentTheme !== 'silence') {
  ...
  playCageShatter() {
    ...
    try {
      const now = this.ctx.currentTime;
      [587.33, 880.00, 1174.66, 1760.00].forEach((freq, i) => {
        const osc = this.ctx.createOscillator();
        const gain = this.ctx.createGain();
  ...
  playGameOver() {
    if (this.muted || !this.ctx) return;
    const now = this.ctx.currentTime;
    [220, 207.65, 196.00, 185.00].forEach((freq, idx) => {
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
      osc.type = 'sawtooth';
  ...
  // Atmospheric background synthesizer
  startBGM(theme) {
    ...
    const tempo = isBoss ? 160 : 380; // ms per note

    // Chord progressions
    const ambientNotes = [110, 130.81, 164.81, 196.00, 146.83, 174.61, 220.00];
    const bossNotes = [65.41, 73.42, 82.41, 98.00, 110.00, 123.47, 130.81];

    this.bgmTimer = setInterval(() => {
      if (this.muted || !this.ctx) return;
      const now = this.ctx.currentTime;
      const notes = isBoss ? bossNotes : ambientNotes;
      ...
      gain.gain.setValueAtTime(isBoss ? 0.08 : 0.04, now);
